...
void SteppingAction::UserSteppingAction(const G4Step* step) {
  ...
  if(volume_Name == "Scattering_foil")   id_Detector = 0;
    ...
    id_Detector = 100 + atoi(tmpStr);
    G4cout << id_Detector;
  }  
  else if(volume_Name.contains("Test_Volume_V_")) {           
    tmpStr = volume_Name; tmpStr.remove(0,11);
    id_Detector = 200 + atoi(tmpStr);
  ...
  G4bool Scoring_Volumes = (id_Detector > 99 && id_Detector < 200);  // Step in a scoring volume
  G4bool Test_Volumes = (id_Detector > 199 && id_Detector < 300);   // Step in a test volume
  
  if(Scoring_Volumes){
    if(partID == 0){
        G4cout << eDep << "    " << volume_Name << G4endl;
    }
  }
  ...
  if(Scoring_Volumes) { // proton
  //if(1) { // store all volumes
