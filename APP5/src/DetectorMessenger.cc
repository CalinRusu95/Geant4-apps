...
{ 
  ...
  fSizeXCmd = new G4UIcmdWithADoubleAndUnit("/testem/det/setSizeZ",this);
  fSizeXCmd->SetGuidance("Set sizeZ of the absorber");
  fSizeXCmd->SetParameterName("SizeZ",false);
  fSizeXCmd->SetRange("SizeZ>0.");
  fSizeXCmd->SetUnitCategory("Length");
  fSizeXCmd->AvailableForStates(G4State_PreInit);
  
  fSizeYZCmd = new G4UIcmdWithADoubleAndUnit("/testem/det/setSizeXY",this);
  fSizeYZCmd->SetGuidance("Set sizeXY of the absorber");
  fSizeYZCmd->SetParameterName("SizeXY",false);
  fSizeYZCmd->SetRange("SizeXY>0.");
...
{ 
  ...
  if( command == fSizeXCmd )
   { fDetector->SetSizeZ(fSizeXCmd->GetNewDoubleValue(newValue));}
   
  if( command == fSizeYZCmd )
   { fDetector->SetSizeXY(fSizeYZCmd->GetNewDoubleValue(newValue));}
  ...
  if (command == fTalDefCmd)
   {
	 ...
	 is >> num >> size >> unt;     
	 fDetector->SetTallySize(num,size);
   }
